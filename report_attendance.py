...
class Report:
    def __init__(self,root):
        ...
        img3 = img3.resize((1350, 700), PIL.Image.LANCZOS)
        ...
        # img_student = img_student.resize((50, 50), PIL.Image.LANCZOS)
        ...
        img_att = img_att.resize((50, 50), PIL.Image.LANCZOS)
        ...
        #topleft
        late_group=LabelFrame(Left_frame,bd=1,bg="white",text="Học sinh đi muộn",font=("times new roman", 11, "bold"),fg="black",relief=RIDGE)
        late_group.place(x=0,y=0,width=630,height=220)

        self.var_com_searchlate = StringVar()
        search_combo = ttk.Combobox(late_group, font=("times new roman", 11, "bold"),
                                    textvariable=self.var_com_searchlate,
                                    state="read only",
                                    width=11)
        search_combo["values"] = ("ID Học sinh", "Ngày","Tên môn học","ID Buổi học","Lớp")
        ...
        searchtc_btn = Button(late_group, text="Tìm kiếm",
                              font=("times new roman", 10, "bold"), bg="#38a6f0", fg="white",command=self.search_Latedata,
                              width=10)
        searchtc_btn.grid(row=0, column=2, padx=5)

        showAlltc_btn = Button(late_group, text="Xem tất cả",
                               font=("times new roman", 10, "bold"), bg="#38a6f0",command=self.fetch_Latedata,
                               fg="white",
                               width=10)
        showAlltc_btn.grid(row=0, column=3, padx=5)

        exportLate_btn = Button(late_group, text="Xuất CSV",
                               font=("times new roman", 10, "bold"), bg="#38a6f0", command=self.exportCsv,
                               fg="white",
                               width=10)
        ...
        self.LateTable = ttk.Treeview(tabletc_frame, column=(
            "studentid", "name","class","date", "subname","lessonid", "status"),
        ...
        #===========under-left===============
        noatt_group = LabelFrame(Left_frame, bd=1, bg="white", text="Học sinh vắng",
                                font=("times new roman", 10, "bold"), fg="black", relief=RIDGE)
        noatt_group.place(x=0, y=235, width=630, height=220)

        self.var_com_searchnoatt = StringVar()
        search_combo = ttk.Combobox(noatt_group, font=("times new roman", 10, "bold"),
        ...
        searchnoatt_btn = Button(noatt_group, text="Tìm kiếm",
                              font=("times new roman", 10, "bold"), bg="#38a6f0", fg="white",command=self.search_Notdata,
                              width=10)
        searchnoatt_btn.grid(row=0, column=2, padx=5)

        showAllnoatt_btn = Button(noatt_group, text="Xem tất cả",
                               font=("times new roman", 10, "bold"), bg="#38a6f0",command=self.fetch_Notdata,
                               fg="white",
                               width=10)
        showAllnoatt_btn.grid(row=0, column=3, padx=5)

        exportNoatt_btn = Button(noatt_group, text="Xuất CSV",
                                font=("times new roman", 10, "bold"), bg="#38a6f0", command=self.exportUnpresetCsv,
                                fg="white",
                                width=10)
        ...
        notinGroup.place(x=0,y=35,width=600,height=420)
        ...
        self.NotInTable = ttk.Treeview(tablenotin_frame, column=(
            "studentid", "name","class", "date", "subname", "lessonid"),
                                      xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
    ...
    def fetch_Latedata(self):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='face_recognizer',
                                           port='3306')
            my_cursor = conn.cursor()
            my_cursor.execute("select DISTINCT student.Student_id,student.`Name`,student.Class,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson,student where   AttendanceStatus like '%Đi muộn%' "
    ...
    def search_Latedata(self):
        if self.var_com_searchlate.get()=="" or self.var_searchlate.get()=="":
            ...
            try:
                ...
                mydata.clear()
                my_cursor.execute("select DISTINCT student.Student_id,student.`Name`,student.Class,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson,student where"
    ...
    def exportCsv(self):
        try:
            ...
            with open('Attendance_CSV/diemdanhmuon.csv',mode="w",newline="",encoding="utf-8") as myfile:
                ...
                exp_write.writerow(('ID Học sinh', 'Tên Học sinh','Lớp', 'Ngày', 'Môn học', 'ID Buổi học', 'Trạng thái'))
                for i in mydata:
    ...
    def fetch_Notdata(self):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='face_recognizer',
                                           port='3306')
            my_cursor = conn.cursor()
            my_cursor.execute("select DISTINCT student.Student_id,student.`Name`,student.class,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson,student where   AttendanceStatus like '%Vắng%' "
                              "and lesson.Subject_id=subject.Subject_id and attendance.Lesson_id=lesson.Lesson_id")
            data = my_cursor.fetchall()
            if len(data) != 0:
    ...
    def fetch_Notindata(self):
            ...
            conn = mysql.connector.connect(host='localhost', user='root', password='', database='face_recognizer',
                                           port='3306')
            my_cursor = conn.cursor()
            my_cursor.execute("select student.Student_id,Name,student.class,Date,Subject_name,Lesson_id from student,lesson,student_has_subject,`subject` "
    ...
    def search_Notindata(self):
        if self.var_com_searchNotin.get()=="" or self.var_searchNotin.get()=="":
            ...
            try:
                ...
                mydataNotInAtt.clear()
                my_cursor.execute("select student.Student_id,Name,student.class,Date,Subject_name,Lesson_id from student,lesson,student_has_subject,`subject` where "
    ...
    def exportNotinCsv(self):
        try:
            ...
            with open('Attendance_CSV/khongdiemdanh.csv',mode="w",newline="",encoding="utf-8") as myfile:
                ...
                exp_write=csv.writer(myfile,delimiter=",")
                exp_write.writerow(('ID Học sinh', 'Tên Học sinh','Lớp', 'Ngày', 'Môn học', 'ID Buổi học'))
