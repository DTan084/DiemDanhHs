...
class Lesson:
    def __init__(self,root):
        ...
        self.root.title("Quản lý thông tin lịch học")
        ...
        dob_label = Label(left_inside_frame, text="Ngày bắt đầu:", font=("times new roman", 12, "bold"),
                          bg="white")
        ...
        teacherLabel = Label(left_inside_frame, text="ID giáo viên:", font=("times new roman", 11, "bold"),
                           bg="white")
        ...
        teachernLabel = Label(left_inside_frame, text="Tên giáo viên:", font=("times new roman", 11, "bold"),
                             bg="white")
        ...
        Right_frame = LabelFrame(main_frame, bd=2, bg="white",
                                 font=("times new roman", 12, "bold"))
        Right_frame.place(x=430, y=5, width=765, height=580)
        ...
        table_frame.place(x=5, y=55, width=760, height=510)
        ...
        self.AttendanceReportTable.heading("teacherid", text="ID giáo viên")
        self.AttendanceReportTable.heading("subjectid", text="ID Môn học")

        self.AttendanceReportTable["show"] = "headings"

        self.AttendanceReportTable.column("id", width=50)
        self.AttendanceReportTable.column("timestart", width=50)
        self.AttendanceReportTable.column("timeend", width=50)
        self.AttendanceReportTable.column("date", width=50)
        self.AttendanceReportTable.column("teacherid", width=50)
        self.AttendanceReportTable.column("subjectid", width=50)
    ...
    def add_data(self):
        ...
        if self.var_id.get()=="" or self.var_timestart.get()=="" or self.var_timeend.get()=="" \
            ...
            messagebox.showerror("Error","Không tồn tại ID giáo viên này! Vui lòng kiểm tra lại!",parent=self.root)
            ...
            try:
                ...
                subject_id = self.var_subjectid.get()
                my_cursor.execute("SELECT credit FROM subject WHERE subject_id = %s", (subject_id,))
                ...
                if result:
                    ...
                    my_cursor.execute("SELECT MAX(Lesson_id) FROM lesson")
                    max_lesson_id = my_cursor.fetchone()[0] or 0
                    
                    for week in range(weeks):
                        lesson_date = self.dob_entry.get_date() + timedelta(weeks=week)
                        new_lesson_id = max_lesson_id + 1 + week
                        my_cursor.execute("INSERT INTO lesson (Lesson_id, Time_start, Time_end, Date, Teacher_id, Subject_id) VALUES (%s, %s, %s, %s, %s, %s)", (
                            new_lesson_id,
                    ...
                    self.reset_data()
                    conn.close()
                    messagebox.showinfo("Thành công", "Thêm thông tin giáo viên thành công", parent=self.root)
                else:
                    messagebox.showerror("Error", "Không tìm thấy thông tin môn học", parent=self.root)
