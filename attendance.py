...
class Attendance:
    ...
    def __init__(self,root):
        ...
        self.root.title("Quản lý thông tin điểm danh")
        ...
        img3 = img3.resize((1350, 700), Image.Resampling.LANCZOS)
        ...
        img_time = img_time.resize((27, 27), Image.Resampling.LANCZOS)
        ...
        label_Update_att = Button(Left_frame, bg="black", fg="white", text="Xem ảnh",command=self.openImage,
                                 font=("times new roman", 12, "bold"))
        label_Update_att.place(x=100, y=480, width=220, height=35)
        update_btn = Button(Left_frame, text="Xóa", command=self.delete_data,
                            font=("times new roman", 12, "bold"),
                            bg="red", fg="white", width=250)
        update_btn.place(x=100, y=530,width=220,height=35 )


        btn_frame = Frame(left_inside_frame, bg="white")
        btn_frame.place(x=0, y=320, width=400, height=105)

        save_btn = Button(btn_frame, text="Nhập file CSV",command=self.importCsv ,font=("times new roman", 11, "bold"), bg="#38a6f0",
                          fg="white", width=16)
        ...
        search_btn = Button(Right_frame,command=self.search_data, text="Tìm kiếm", font=("times new roman", 11, "bold"), bg="#38a6f0", fg="white",
                            width=12)
        search_btn.grid(row=0, column=3, padx=15)

        Today_btn = Button(Right_frame, text="Hôm nay",command=self.today_data, font=("times new roman", 11, "bold"), bg="#38a6f0",
                             fg="white",
                             width=12)
        Today_btn.grid(row=0, column=4, padx=15)


        showAll_btn = Button(Right_frame, text="Xem tất cả", command=self.fetch_data,font=("times new roman", 11, "bold"), bg="#38a6f0",
                             fg="white",
                             width=12)
    ...
    def exportCsv(self):
        # try:
            ...
            with open('Attendance_CSV/diemdanh.csv',mode="w",newline="",encoding="utf-8") as myfile:
                ...
                exp_write.writerow(('Mã Điểm danh', 'ID Học sinh', 'Tên Học sinh', 'Lớp biên chế', 'Giờ vào', 'Giờ ra', 'Ngày', 'ID Buổi học', 'Trạng thái'))
                for i in mydata:
