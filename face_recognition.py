...
class Face_Recognition:
    ...
    def __init__(self,root):
        ...
        Left_frame.place(x=30, y=64, width=780, height=540)
        ...
        self.notify_frame = LabelFrame(Left_frame, bd=1, bg="white", relief=RIDGE,
                                       font=("times new roman", 11, "bold"))
        self.notify_frame.place(x=8, y=475, width=760, height=40)
        ...
        img_btn1 = img_btn1.resize((350, 45), PIL.Image.LANCZOS)
        ...
        self.btnClose.place(x=460, y=607, width=350, height=30)

        #Right_frame
        self.Right_frame = LabelFrame(self.root, bd=2, bg="white", relief=RIDGE, text="Điểm danh thành công",
                                font=("times new roman", 12, "bold"))
        self.Right_frame.place(x=850, y=63, width=400, height=310)

        self.img_right = PIL.Image.open(r"ImageFaceDetect\unknow.jpg")
        self.img_right = self.img_right.resize((150, 150), PIL.Image.LANCZOS)
        ...
        self.f_lbl.place(x=110, y=10, width=150, height=150)

        self.studentID_atten_info=Label(self.Right_frame, bg="white",
                                font=("times new roman", 12, "bold"))
        self.studentID_atten_info.place(x=5, y=170, width=380, height=120)
        ...
        self.RightU_frame = LabelFrame(self.root, bd=2, bg="white", relief=RIDGE, text="Thông tin buổi học",
                                      font=("times new roman", 11, "bold"))
        self.RightU_frame.place(x=850, y=380, width=400, height=160)
        ...
        # Listbox to Display Students Attendance
        self.RightUb_frame = LabelFrame(self.root, bd=2, bg="white", relief=RIDGE, text="Danh sách sinh viên đã điểm danh",
                                        font=("times new roman", 11, "bold"))
        self.RightUb_frame.place(x=850, y=550, width=400, height=180)
        
        self.listbox_students = Listbox(self.RightUb_frame, font=("times new roman", 14), relief=RIDGE)
        self.listbox_students.place(x=0, y=0, width=390, height=180)
        
        #Kiem tra xem hom nay co mon hoc can diem danh khuon mat hay khong
    ...
    def load_subject(self):
        lesson_info = self.lesson_combo.get()
        if lesson_info:
            lesson_id = lesson_info.split(",")[1]
            conn = mysql.connector.connect(host='localhost', user='root', password='',
                                           database='face_recognizer', port='3306')
            my_cursor = conn.cursor()
            my_cursor.execute("SELECT student.Student_id, student.Name FROM attendance "
                              "JOIN student ON attendance.Student_id = student.Student_id "
                              "WHERE attendance.Lesson_id = %s", (lesson_id,))
            students = my_cursor.fetchall()
            self.listbox_students.delete(0, END)
            for student in students:
                self.listbox_students.insert(END, f"{student[0]} - {student[1]}")
            conn.close()
    ...
    def mark_attendance(self,i,r,n,d,face_cropped):
        ...
        while True:# khi camera mở lên không có lỗi
            ...
            # kiểm tra xem Học sinh có trong ds lớp hay không
            conn = mysql.connector.connect(host='localhost', user='root', password='',
                                           database='face_recognizer', port='3306')
            my_cursor = conn.cursor()
            my_cursor.execute(
                ...
                try:
                        ...
                        if(self.type_attendance.get()=="Vào"):
                            if((d1 not in a)) or ((self.lessonid not in b)):
                                ...
                                self.img_right = self.img_right.resize((190, 190), PIL.Image.LANCZOS)
                                self.photoimg_left = ImageTk.PhotoImage(self.img_right)
                                ...
                                self.notify_label['text'] = "Thông báo: Học sinh: " + n + " đã điểm danh vào lớp thành công môn học "+self.subject_name
                            ...
                            if(masp not in att):
                                if ((d1 not in a)) or ((self.lessonid not in b)):
                                    ...
                                    self.notify_label[
                                        'text'] = "Thông báo: Học sinh: " + n + " đã điểm danh ra thành công môn học " + self.subject_name
                                    self.notify_label['fg'] = "green"
